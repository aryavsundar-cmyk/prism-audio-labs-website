...
import { collections, totalPluginCount, completeSuitePrice } from '@/data/plugins';
...
export default function PricingPage() {
  ...
  return (
    <div className="pt-28">
      ...
      {/* Complete Suite — featured at top */}
      <section className="pb-16">
            ...
            <div className="flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-base text-slate-400">
                  All {totalPluginCount} plugins across all four collections. One purchase, unlimited creativity.
                </p>
                <div className="mt-4 flex flex-wrap gap-3">
                  {collections.map((c) => (
                    <span key={c.id} className={`rounded-full bg-white/[0.04] px-3 py-1 text-xs text-slate-400`}>
              ...
              <div className="shrink-0 text-center md:text-right">
                ...
                <div className="mt-1 text-sm text-slate-400 line-through">$726</div>
                <div className="mt-1 text-sm text-prism-cyan/60">Save $327</div>
                <button
      ...
      {/* Individual Collections — 2x2 grid */}
      <section className="pb-24">
        <div className="mx-auto max-w-7xl px-6">
          <h3 className="mb-6 text-lg font-semibold text-slate-300">Or choose a collection</h3>
          <div className="grid gap-6 sm:grid-cols-2">
            {collections.map((col) => {
              const inCart = isInCart(`collection-${col.id}`) || isInCart('complete-suite');
              return (
                <div key={col.id} className="rounded-2xl border border-white/[0.06] bg-[#14152E] p-10">
